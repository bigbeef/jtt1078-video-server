...
public class Jtt1078Handler extends SimpleChannelInboundHandler<Packet> {
    ...
    public Jtt1078Handler(String tagSuffix) {
        this.tagSuffix = (tagSuffix == null) ? "" : tagSuffix;
    }
    ...
    protected void channelRead0(ChannelHandlerContext ctx, Packet packet) throws Exception {
        ...
        String tag = sim + "-" + channel + tagSuffix;

        Session session = getSession();
        if (null == session) {
            setSession(session = new Session());
            Channel chl = PublishManager.getInstance().open(tag, ctx);
            session.set("tag", tag);
